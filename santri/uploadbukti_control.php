  ...
  if (isset($_POST['upload'])) {
    ...
    if(in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
      if($ukuran < 1044070) {
        ...
        $sql_update_profil = "UPDATE pendaftar set status_pembayaran = '1', upload_bukti = '$ubah_nama' where id='$id_pendaftar'";
        $query_update = mysqli_query($conn, $sql_update_profil);

        if($query_update) {
          $_SESSION['pesan_bukti'] = "Upload Bukti Pembayaran Berhasil";
          header('location:konfirmasi_pembayaran.php');    
        } else {
          $_SESSION['pesan_bukti'] = "Upload Bukti Pembayaran Gagal";
          die();
          header('location:konfirmasi_pembayaran.php');
        }
      } else {
        $_SESSION['pesan_bukti'] = "Gambar terlalu besar";
        die();
        header('location:konfirmasi_pembayaran.php');    
      }
    } else {
      $_SESSION['pesan_bukti'] = "Ekstensi tidak diperbolehkan";
      die();
      header('location:konfirmasi_pembayaran.php');    
    }
  }
